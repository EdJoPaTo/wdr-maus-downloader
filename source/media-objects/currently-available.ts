...
import { ErrorHandler, matchAll, sequentialAsync } from "../generics.ts";
...
async function getMediaObjectsFromSource(source: string): Promise<any[]> {
  ...
  const allResponses = await sequentialAsync(
...
function createEntries(
  ...
  mediaObjects: readonly any[],
): Entry[] {
  if (mediaObjects.length !== imageUrls.length) {
    throw new Error(
      `should find the same amount of images and video media objects ${imageUrls.length} != ${mediaObjects.length}`,
    );
  }
...
async function getAktuelleSendung(
...
async function getCorona(errorHandler: ErrorHandler): Promise<Entry[]> {
  ...
  try {
    ...
    const imageUrls = matchAll(
      /<img src="..\/..\/..\/extras\/mausthemen\/corona\/(imggen\/.+\.jpg)/g,
      body,
    )
      .map((o) => o[1]!)
      .map((o) => BASE_URL + o);
